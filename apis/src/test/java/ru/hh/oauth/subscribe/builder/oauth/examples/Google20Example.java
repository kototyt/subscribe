...
import ru.hh.oauth.subscribe.builder.api.GoogleApi20;
...
public abstract class Google20Example {
  ...
  public static void main(final String[] args) {
    ...
    final OAuth20ServiceImpl service =
      (OAuth20ServiceImpl) new ServiceBuilder().provider(GoogleApi20.class).apiKey(clientId).apiSecret(clientSecret).scope("profile") // replace with desired scope
      .grantType("authorization_code").state(secretState).callback("http://example.com/callback").build();
